...
dataset_dir = "./data/track_output_2__"
data_samples = []
...
for sub_dir in sorted(os.listdir(dataset_dir)):
    ...
    if os.path.isdir(sub_dir_path):
        ...
        with open(input_path, "r") as f:
            perceptions = json.load(f)
            actions = interpret_action_with_speed(perceptions, 0.5)
        ...
        try:
            ...
            context = """Create a Scenic script for placing cars and simulating their behavior, each car must be positioned at specified coordinates using the syntax new Car offset by (x, y), with each assigned the blueprint "vehicle.lincoln.mkz_2020" and color Color(0.3, 0.3, 0.3). Example commands for placement include new Car offset by (2.8, -7.5), with blueprint "vehicle.lincoln.mkz_2020", with color Color(0.3,0.3,0.3). The coordinates for placement are: """
            instruction = context + instruction
            step2 = instruction + " Create the script that defines car behaviors. For instance, a behavior definition might look like: behavior Behavior_car1(): do CustomFollowLaneBehavior(0.80) for 0.5 seconds\n do CustomFollowLaneBehavior(1.92) for 0.5 seconds\n do CustomLaneChangeBehavior(network.laneSectionAt(self)._laneToLeft). Behavior specifications include commands for various cars: "
            step2 = step2 + formatted_func
            step3 = step2 + " These above should be combined into a cohesive Scenic script."
            data_samples.append({"instruction": step3, "response": output }) # final})
            # print(data_samples)
...
output_dir = "carla-code-llama-func-instruct-baseline"
import datetime
